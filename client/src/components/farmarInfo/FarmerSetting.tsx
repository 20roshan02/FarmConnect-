/**
 * FarmerSetting.tsx — Modern redesign
 * Profile update · Password change · Account danger zone
 */
...
const inputCls = "w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 placeholder:text-slate-300";
  ...
  return (
    <div className="relative">
      <input type={show ? "text" : "password"} value={value} onChange={e => onChange(e.target.value)}
        placeholder={placeholder} autoComplete={autoComplete} className={inputCls + " pr-11"} />
...
function SectionCard({ icon: Icon, title, subtitle, children, accent = "emerald" }: {
  icon: any; title: string; subtitle: string; children: React.ReactNode; accent?: "emerald"|"blue"|"red";
}) {
  const map = {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center gap-4 border-b border-slate-100 bg-slate-50/50 px-6 py-4">
        <div className={`flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br ${map[accent]} text-white shadow-sm shrink-0`}>
          <Icon size={18} />
        </div>
        <div>
...
export default function FarmerSettings() {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.put(`${import.meta.env.VITE_API_URL}/user/profile`, { name: name.trim(), email: email.trim() }, { headers: { Authorization: `Bearer ${t}` } });
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      setSavingPw(true);
      await axios.put(`${import.meta.env.VITE_API_URL}/user/change-password`, { currentPassword: currentPw, newPassword: newPw }, { headers: { Authorization: `Bearer ${t}` } });
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <div className="fixed inset-x-0 top-0 z-30 flex items-center justify-between border-b bg-white/90 px-4 py-3 backdrop-blur-md md:hidden">
        ...
        <button onClick={() => setMobileOpen(true)} className="rounded-xl border border-slate-200 p-2 text-slate-600"><HiOutlineMenu size={20} /></button>
      </div>
      ...
      <main className="mt-14 flex-1 overflow-y-auto px-4 py-6 sm:px-6 lg:px-8 md:mt-0">
        <div className="mx-auto max-w-2xl space-y-6">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Manage your profile, security, and account.</p>
          </div>

          {/* Profile */}
          <SectionCard icon={HiOutlineUser} accent="emerald" title="Profile Information" subtitle="Update your name and email">
            <form onSubmit={handleProfileUpdate} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wide text-slate-400">Full Name</label>
                  <input value={name} onChange={e=>setName(e.target.value)} placeholder="Your name" className={inputCls} />
                </div>
                <div>
                  <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wide text-slate-400">Email</label>
          ...
          {/* Password */}
          <SectionCard icon={HiOutlineLockClosed} accent="blue" title="Change Password" subtitle="Use a strong, unique password">
            <form onSubmit={handlePasswordChange} className="space-y-4">
              <div>
                <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wide text-slate-400">Current Password</label>
                <PasswordInput value={currentPw} onChange={setCurrentPw} placeholder="Current password" autoComplete="current-password" />
              </div>
              <div>
                <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wide text-slate-400">New Password</label>
                <PasswordInput value={newPw} onChange={v => { setNewPw(v); setPwStrength(calcStrength(v)); }} placeholder="Min 8 characters" autoComplete="new-password" />
                {newPw.length > 0 && (
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <span className={`text-[11px] font-semibold ${pwStrength<=1?"text-red-500":pwStrength<=2?"text-amber-500":pwStrength<=3?"text-blue-500":"text-emerald-600"}`}>
              ...
              <div>
                <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wide text-slate-400">Confirm New Password</label>
                <PasswordInput value={confirmPw} onChange={setConfirmPw} placeholder="Confirm new password" autoComplete="new-password" />
                {confirmPw.length > 0 && newPw !== confirmPw && <p className="mt-1 text-[11px] text-red-500">Passwords do not match</p>}
                {confirmPw.length > 0 && newPw === confirmPw && newPw.length > 0 && (
                  <p className="mt-1 flex items-center gap-1 text-[11px] font-semibold text-emerald-600"><HiOutlineCheckCircle size={13} /> Passwords match</p>
                )}
              </div>
              <div className="flex justify-end">
          ...
          {/* Danger zone */}
          <SectionCard icon={HiOutlineExclamationTriangle} accent="red" title="Account" subtitle="Logout or permanently delete your account">
              ...
              <div className={`rounded-xl border ${showDelete ? "border-red-300 bg-red-50" : "border-slate-200 bg-slate-50"} px-5 py-4 transition-all`}>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Permanently remove your account and all products</p>
                  </div>
                  {!showDelete && (
                ...
                {showDelete && (
                  <div className="mt-4 space-y-3">
                    <p className="text-sm text-red-700 leading-relaxed">This will <strong>permanently delete</strong> your account, products and all associated data. This cannot be undone.</p>
                    <div>
                      <label className="mb-1 block text-[12px] font-semibold text-red-600">Type <span className="font-mono bg-red-100 px-1 rounded">DELETE</span> to confirm</label>
                      <input value={deleteInput} onChange={e => setDeleteInput(e.target.value)} placeholder="DELETE"
                        className="w-full rounded-xl border border-red-300 bg-white px-4 py-2.5 text-sm font-mono text-red-800 outline-none focus:ring-2 focus:ring-red-300" />
                    </div>
                    <div className="flex gap-3">
                      <button onClick={handleDeleteAccount} disabled={deleting || deleteInput !== "DELETE"}
                        ...
                        {deleting ? <><span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" />Deleting…</> : <><HiOutlineTrash size={15} />Yes, delete</>}
                      </button>
